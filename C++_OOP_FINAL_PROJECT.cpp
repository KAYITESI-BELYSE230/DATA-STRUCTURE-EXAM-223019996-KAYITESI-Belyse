...
struct Edge { int to; float weight; };              // Edge struct: destination and weight
struct Vertex { int id; Edge* edges; int edgeCount; Vertex() : edges(NULL), edgeCount(0) {} }; // Vertex with dynamic edges

class GraphBase {
public:
    Vertex* vertices = NULL; int vertexCount = 0;  // Dynamic array of vertices
    ...
    void addVertex(int id) {                       // Add vertex by resizing array
    ...
    void removeVertex(int id) {                     // Remove vertex by id
        int idx = -1;
        for (int i = 0; i < vertexCount; i++) if (vertices[i].id == id) { idx = i; break; }
        if (idx == -1) return;                      // Vertex not found, do nothing
        delete[] vertices[idx].edges;               // Delete edges of removed vertex

        Vertex* temp = new Vertex[vertexCount - 1]; // New array one smaller
    ...
    void display() {                                // Display vertices and their edges
...
public:
    void addEdge(int from, int to, float w) override { // Add directed edge
        ...
        if (idx == -1) return;                        // Vertex not found
...
int main() {
    ...
    for (int i = 1; i <= V; i++) {                  // Add vertices with IDs 1..V
        graphs[0]->addVertex(i);
        graphs[1]->addVertex(i);
    }

    for (int i = 0; i < E; i++) {                    // Add edges from user input
        int f, t; float w;
    ...
    if (removeId != 0) {                             // Remove vertex if input != 0
